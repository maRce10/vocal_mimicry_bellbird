...
pkgs <- c("rlesur/klippy", "kableExtra", "knitr", "remotes", "maRce10/warbleR", "Rraven", "viridis", "ggplot2", "formatR", "soundgen", "caret")
...
```{r custom data frame printing, echo = FALSE, eval = TRUE}
...
- Measure only on signals with a signal-to-noise ratio above 2 DDB
...
### Train model
```{r train random forest on data subsets using K-folds dialects}
...
saveRDS(list(pred_model_bb = pred_model, conf_mat_bb = conf_mat, confusion_df_bb = conf_df, testset_bb = testset), "./data/processed/random_forest_model_results_olny_bellbird_dialects.RDS")
...
rf_model_results_bb <- readRDS("./data/processed/random_forest_model_results_olny_bellbird_dialects.RDS")
...
```{r predict mimetic dialects, eval = TRUE}
...
```{r predict mimetic sounds keeping dialects, eval = TRUE}
...
```{r predict mimetic sounds no dialects, eval = TRUE}
...
### Train model
```{r train random forest on data subsets using K-folds collapsing dialects}
...
### Predict mimetic bellbird and compare to manual labels
```{r predict mimetic sounds no dialects 2, eval = TRUE}
